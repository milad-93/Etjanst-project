...
$(document).ready(function(){
    ...
    $('#ansokan-form').click(function() {
        url = 'http://localhost:50240/api/RemoveAnsokan?personnummer=' + getUrlParameter('p');
        $.post(
            url, 
            function(data, status){
                if(data){
                    ...
                    window.location.href = "skapaAnsokans.html";
            }
        });
    });
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            var bedomningAvHandlaggare = result[0].BedomningAvHandlaggare;
            $('#bedomningAvHandlaggare-input').val(bedomningAvHandlaggare);

            StatusAvHandlaggare = result[0].StatusAvHandlaggare;
            if(StatusAvHandlaggare){
                // Check #x
                $("#statusAvHandlaggare-input").prop( "checked", true );
                $("#ansokan-form").attr("disabled", true);
            }else{
                // Uncheck #x
                $( "#statusAvHandlaggare-input" ).prop( "checked", false );
            }            
        },
